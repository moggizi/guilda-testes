...
let buyerOrdersUnsub = null;
let buyerChatsUnsub = null;
let sellerOrdersUnsub = null;
let sellerChatsUnsub = null;
let supportChatsUnsub = null;
let supportRefundsUnsub = null;
...
function stopLojinhaRealtimeListeners(scope = '') {
  const shouldStop = (name) => !scope || String(scope) === name;
  if (shouldStop('buyerOrders') && buyerOrdersUnsub) { try { buyerOrdersUnsub(); } catch (_) {} buyerOrdersUnsub = null; }
  if (shouldStop('buyerChats') && buyerChatsUnsub) { try { buyerChatsUnsub(); } catch (_) {} buyerChatsUnsub = null; }
  if (shouldStop('sellerOrders') && sellerOrdersUnsub) { try { sellerOrdersUnsub(); } catch (_) {} sellerOrdersUnsub = null; }
  if (shouldStop('sellerChats') && sellerChatsUnsub) { try { sellerChatsUnsub(); } catch (_) {} sellerChatsUnsub = null; }
  if (shouldStop('supportChats') && supportChatsUnsub) { try { supportChatsUnsub(); } catch (_) {} supportChatsUnsub = null; }
  if (shouldStop('supportRefunds') && supportRefundsUnsub) { try { supportRefundsUnsub(); } catch (_) {} supportRefundsUnsub = null; }
}

function cacheBuyerOrdersPage() {
  if (!ghubProfile?.gameId) return;
  lojaCacheSet('buyerOrders', { orders: buyerOrders, problems: buyerProblems, ratings: buyerRatings, hasNextPage: buyerOrdersHasNextPage, page: buyerOrdersPage }, `${ghubProfile.gameId}:page:${buyerOrdersPage || 1}`);
}

function cacheSellerOrdersPage() {
  if (!ghubProfile?.gameId) return;
  lojaCacheSet('sellerOrders', { orders: sellerOrders, hasNextPage: sellerOrdersHasNextPage, page: sellerOrdersPage }, `${ghubProfile.gameId}:page:${sellerOrdersPage || 1}`);
}

function applySellerOrderLocalPatch(orderId, patch = {}) {
  const cleanId = String(orderId || '');
  if (!cleanId) return;
  sellerOrders = sellerOrders.map((item) => String(item.id) === cleanId ? { ...item, ...patch } : item);
...
function applyBuyerOrderLocalPatch(orderId, patch = {}) {
  const cleanId = String(orderId || '');
  if (!cleanId) return;
  buyerOrders = buyerOrders.map((item) => String(item.id) === cleanId ? { ...item, ...patch } : item);
  cacheBuyerOrdersPage();
  renderBuyerOrders();
  updateNotificationBadges();
}

function startBuyerOrdersRealtime() {
  ...
  try {
    ...
    buyerOrdersUnsub = onSnapshot(q, (snap) => {
      ...
      console.warn('Realtime das compras indisponível; usando cache/leitura manual:', err?.code || err?.message || err);
      stopLojinhaRealtimeListeners('buyerOrders');
    });
  } catch (err) {
    console.warn('Não foi possível iniciar realtime das compras:', err?.code || err?.message || err);
  }
}

function startBuyerChatsRealtime() {
  if (!currentUser || !ghubProfile?.gameId || buyerChatsUnsub) return;
  const buyerId = String(ghubProfile.gameId);
  try {
    const q = query(collection(lojaDb, CHAT_COLLECTION), where('buyerId', '==', buyerId), limit(30));
    buyerChatsUnsub = onSnapshot(q, async (snap) => {
      ...
      console.warn('Realtime dos chats do comprador indisponível:', err?.code || err?.message || err);
      stopLojinhaRealtimeListeners('buyerChats');
    });
  } catch (err) {
    console.warn('Não foi possível iniciar realtime dos chats do comprador:', err?.code || err?.message || err);
  }
}

function startSellerOrdersRealtime() {
  ...
  try {
    ...
    sellerOrdersUnsub = onSnapshot(q, (snap) => {
      ...
      console.warn('Realtime dos pedidos do vendedor indisponível; usando cache/leitura manual:', err?.code || err?.message || err);
      stopLojinhaRealtimeListeners('sellerOrders');
    });
  } catch (err) {
    console.warn('Não foi possível iniciar realtime dos pedidos do vendedor:', err?.code || err?.message || err);
  }
}

function startSellerChatsRealtime() {
  if (!currentUser || !ghubProfile?.gameId || sellerChatsUnsub) return;
  const sellerId = String(ghubProfile.gameId);
  try {
    const q = query(collection(lojaDb, CHAT_COLLECTION), where('sellerId', '==', sellerId), limit(30));
    sellerChatsUnsub = onSnapshot(q, async (snap) => {
      sellerChats = snap.docs.map(normalizeChatDoc);
      await markExpiredChatsIfNeeded(sellerChats);
      lojaCacheSet('sellerChats', sellerChats, sellerId);
      renderSellerSupportChats();
      updateNotificationBadges();
    }, (err) => {
      console.warn('Realtime dos chats do vendedor indisponível:', err?.code || err?.message || err);
      stopLojinhaRealtimeListeners('sellerChats');
    });
  } catch (err) {
    console.warn('Não foi possível iniciar realtime dos chats do vendedor:', err?.code || err?.message || err);
  }
}
...
function startSupportChatsRealtime() {
  ...
  try {
    ...
    supportChatsUnsub = onSnapshot(q, async (snap) => {
      ...
      console.warn('Realtime dos chats de suporte indisponível:', err?.code || err?.message || err);
      stopLojinhaRealtimeListeners('supportChats');
    });
  } catch (err) {
    console.warn('Não foi possível iniciar realtime dos chats de suporte:', err?.code || err?.message || err);
  }
}
...
function startSupportRefundsRealtime() {
  ...
  try {
    ...
    supportRefundsUnsub = onSnapshot(q, (snap) => {
      ...
      console.warn('Realtime dos reembolsos indisponível:', err?.code || err?.message || err);
      stopLojinhaRealtimeListeners('supportRefunds');
    });
  } catch (err) {
    console.warn('Não foi possível iniciar realtime dos reembolsos:', err?.code || err?.message || err);
  }
}
...
async function loadBuyerProblems() {
  ...
  try {
    const snap = await getDocs(query(collection(lojaDb, CHAT_COLLECTION), where('buyerId', '==', String(ghubProfile.gameId)), limit(30)));
    buyerProblems = snap.docs.map(normalizeChatDoc);
    await markExpiredChatsIfNeeded(buyerProblems);
    updateNotificationBadges();
    startBuyerChatsRealtime();
  } catch (err) {
...
async function loadSellerChats(force = false) {
  ...
  if (Array.isArray(cached)) {
    sellerChats = cached;
    updateNotificationBadges();
    startSellerChatsRealtime();
    return;
  }
  try {
    const snap = await getDocs(query(collection(lojaDb, CHAT_COLLECTION), where('sellerId', '==', String(ghubProfile.gameId)), limit(30)));
    sellerChats = snap.docs.map(normalizeChatDoc);
    await markExpiredChatsIfNeeded(sellerChats);
