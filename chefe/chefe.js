import { checkAuth, setupSidebar, initIcons, logout, db, ensureCeoStatus, applyCeoNavVisibility, showToast, getVipTier } from '../logic.js';
    import { collection, getDocs, getDoc, doc, updateDoc, deleteDoc, writeBatch, query, where, serverTimestamp, setDoc, runTransaction } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
    ...
    function prettyVipName(v) {
      ...
      return t === 'vitalicio' ? 'VITALÍCIO' : t.toUpperCase();
    }
    ...
    function buildVipExpiresAt(tier) {
      ...
      if (!days) return null;
      if (days === 'forever') return Date.UTC(9999, 11, 31, 23, 59, 59, 999);
    ...
    function remainingTag(expiresAtMs, tier) {
      const t = normalizeVip(tier);
      if (t === 'vitalicio') return { text: 'Vitalício ativo', cls: 'bg-amber-100 text-amber-700' };
    ...
    async function loadData(force = false) {
      ...
      if (!force && cachedGuildas && cachedConfig) {
        try {
          ...
          if (Date.now() - pGuildas.ts < 30 * 60 * 1000) { // 30 minutos
    ...
    function setActiveView(view) {
      currentAdminView = view === 'partners' ? 'partners' : 'guildas';
      const isPartners = currentAdminView === 'partners';
      document.getElementById('guilds-grid')?.classList.toggle('hidden', isPartners);
      ...
      if (btnGuildas && btnPartners) {
        btnGuildas.className = isPartners
    ...
    async function loadPartners(force = false) {
      ...
      if (!force && cached) {
        try {
          ...
          if (parsed?.items && (Date.now() - Number(parsed.ts || 0)) < 10 * 60 * 1000) {
    ...
    function renderPartners() {
      const q = document.getElementById('guild-search').value.toLowerCase().trim();
      ...
      grid.innerHTML = filtered.map((p) => {
        ...
        return `
          <div class="bg-white border border-gray-100 rounded-3xl p-5 shadow-sm hover:shadow-xl transition-all duration-300">
    ...
    async function applyVipToGuild(guildId, tier) {
      ...
      const expiresAtMs = buildVipExpiresAt(normalized);
      const patch = {
        vipTier: normalized,
    ...
    window.setPartnerVerified = async (partnerId) => {
      ...
      if (!confirm(`Tornar ${p.nick || p.email || p.id} parceiro verificado e liberar vitalício na guilda ${p.guildName || p.guildId}?`)) return;
      ...
      try {
        ...
        await applyVipToGuild(p.guildId, 'vitalicio');
        await updatePartnerLocalAndCache(partnerId, { ...patch, guildVip: 'vitalicio', guildVipExpiresAt: buildVipExpiresAt('vitalicio') });
        showToast('success', 'Parceiro verificado e guilda liberada como vitalício.');
      } catch (e) {
        console.error(e);
    ...
    window.openPartnerDetails = async (partnerId) => {
      ...
      try {
        ...
        comissoes = cSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
      } catch (_) {}
    ...
    function renderGuilds() {
      ...
      const filtered = allGuilds.filter(g => {
        ...
        return matchesQ && matchesVip && matchesUpdated;
      });
      ...
      grid.innerHTML = filtered.map(g => `
        <div class="bg-white border border-gray-100 rounded-3xl p-5 shadow-sm hover:shadow-xl transition-all duration-300 group">
          <div class="flex items-start justify-between mb-4">
            ...
            <span class="px-3 py-1 rounded-xl ${normalizeVip(g.vip) === 'vitalicio' ? 'bg-amber-100 text-amber-700' : (normalizeVip(g.vip) === 'business' ? 'bg-purple-100 text-purple-700' : (normalizeVip(g.vip) === 'pro' ? 'bg-blue-100 text-blue-700' : (normalizeVip(g.vip) === 'plus' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600')))} text-[10px] font-black uppercase tracking-wider shadow-sm border border-black/5">
              ${prettyVipName(g.vip)}
            </span>
    ...
    window.openDetails = (gid) => {
      ...
      document.getElementById('drawer-content').innerHTML = `
        <div class="space-y-6 animate-in slide-in-from-right duration-300">
          <div class="p-4 bg-gray-50 rounded-2xl border border-gray-100">
             ...
             <select id="edit-vip-sel" class="w-full p-3 rounded-xl border border-gray-200 bg-white text-sm font-bold">
                <option value="free" ${normalizeVip(g.vip)==='free'?'selected':''}>FREE</option>
                <option value="plus" ${normalizeVip(g.vip)==='plus'?'selected':''}>PLUS</option>
                <option value="pro" ${normalizeVip(g.vip)==='pro'?'selected':''}>PRO</option>
                <option value="business" ${normalizeVip(g.vip)==='business'?'selected':''}>BUSINESS</option>
                <option value="vitalicio" ${normalizeVip(g.vip)==='vitalicio'?'selected':''}>VITALÍCIO</option>
             </select>
      ...
      document.getElementById('btn-save-vip').onclick = async () => {
        ...
        try {
          const expiresAtMs = buildVipExpiresAt(nv);
          const patch = { vipTier: nv, vipExpiresAt: expiresAtMs, updatedAt: serverTimestamp(), updatedAtMs: Date.now() };
          ...
          localStorage.setItem('hub_ceo_guildas_v1', JSON.stringify({ ts: Date.now(), items: window.allGuildas }));
        } catch(e) { console.error('VIP save error:', e); showToast('error', 'Erro ao salvar VIP.'); }
      };
    ...
    window.fullDeleteGuild = async (gid, name) => {
      if(!confirm(`⚠️ REALMENTE QUER EXCLUIR TUDO DA GUILDA: ${name}?\n\nIsso apagará Membros, Lines, Campeonatos, Configurações e todos os UTILIZADORES vinculados ao ID da guilda.\n\nEsta ação é irreversível!`)) return;
      ...
      try {
        ...
        try {
          ...
          memSnap.forEach((d) => refsToDelete.push(d.ref));
        } catch (e) {}

        try {
          const lineSnap = await getDocs(collection(db, 'guildas', gid, 'lines'));
          lineSnap.forEach((d) => refsToDelete.push(d.ref));
        } catch (e) {}

        try {
          const campSnap = await getDocs(collection(db, 'guildas', gid, 'campeonatos'));
          campSnap.forEach((d) => refsToDelete.push(d.ref));
        } catch (e) {}

        try {
          const userSnap = await getDocs(query(collection(db, 'users'), where('guildId', '==', gid)));
          userSnap.forEach((d) => refsToDelete.push(d.ref));
        } catch (e) {}
        ...
        try {
          ...
          allGuilds = window.allGuildas;
          window.allSolicita = (window.allSolicita || []).filter(s => (s.guildId || s.gid) !== gid);
        } catch(_) {}
    ...
    window.approveSolicita = async (sid, plano) => {
      try {
        ...
        const expiresAtMs = buildVipExpiresAt(normalizedPlano);
    ...
    document.getElementById('guild-search').oninput = () => currentAdminView === 'partners' ? renderPartners() : renderGuilds();
    document.getElementById('vip-filter').onchange = () => currentAdminView === 'partners' ? renderPartners() : renderGuilds();
    document.getElementById('updated-filter').onchange = () => currentAdminView === 'partners' ? renderPartners() : renderGuilds();
