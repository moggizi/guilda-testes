  ...
  els.supportRefundsList.innerHTML = supportRefundRequests.map((order) => {
    ...
    return `<div class="rounded-2xl border border-amber-100 bg-amber-50 p-3">
      ...
      <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
        <button type="button" data-support-refund-order="${escapeHtml(order.id)}" class="rounded-xl bg-red-600 px-3 py-2 text-xs font-black text-white hover:bg-red-700">Marcar reembolsado</button>
        <button type="button" data-support-refund-chat="${escapeHtml(order.id)}" class="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-black text-sky-700 hover:bg-sky-100">Abrir chat do pedido</button>
      </div>
    </div>`;
  }).join('');
