...
function renderRecruitment() {
  ...
  const guildTypes = Array.isArray(data.guildType) && data.guildType.length ? data.guildType.map(v => `<span class="inline-flex items-center gap-1 rounded-full bg-fuchsia-50 text-fuchsia-700 ring-1 ring-fuchsia-200 px-3 py-1 text-xs font-semibold">${escapeHtml(v)}</span>`).join(' ') : '<span class="text-sm text-gray-400">Nenhum</span>';
  const focuses = Array.isArray(data.focus) && data.focus.length ? data.focus.map(v => `<span class="inline-flex items-center gap-1 rounded-full bg-sky-50 text-sky-700 ring-1 ring-sky-200 px-3 py-1 text-xs font-semibold">${escapeHtml(v)}</span>`).join(' ') : '<span class="text-sm text-gray-400">Nenhum</span>';
