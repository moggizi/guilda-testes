...
function formatBadges(values, tone = 'gray') {
  if (!Array.isArray(values) || !values.length) return '<span class="text-sm text-gray-400">Nenhuma</span>';
  const cls = tone === 'emerald'
    ? 'bg-emerald-50 text-emerald-700 ring-emerald-200'
    : 'bg-gray-100 text-gray-700 ring-gray-200';
  return values.map(v => `<span class="inline-flex items-center gap-1 rounded-full ${cls} ring-1 px-3 py-1 text-xs font-semibold">${escapeHtml(v)}</span>`).join(' ');
}
...
function openModal(mode = 'create') {
  ...
  if (mode === 'edit' && currentRecruitment) {
    ...
    setCheckedValues('guildType', []);
    setCheckedValues('focus', []);
  }
...
function renderRecruitment() {
  ...
  if (!currentRecruitment) {
    els.view.innerHTML = `
      <div class="bg-white rounded-2xl p-10 border border-gray-100 shadow-sm text-center">
        ...
        <p class="text-gray-800 font-semibold">Nenhum recrutamento foi criado ainda</p>
        <p class="text-gray-500 text-sm mt-1">Você já pode criar o recrutamento dessa guilda agora.</p>
      </div>`;
    updateCreateButtonVisibility();
    initIcons();
    return;
  ...
  const roles = formatBadges(data.roles, 'emerald');
  const contacts = formatBadges(data.contacts, 'gray');
  const guildType = formatBadges(data.guildType, 'gray');
  const focus = formatBadges(data.focus, 'gray');
  ...
  els.view.innerHTML = `
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div class="p-5 grid md:grid-cols-2 gap-5">
        ...
        <div>
          <p class="text-xs font-semibold text-gray-500 mb-2">Contato</p>
          <div class="flex flex-wrap gap-2">${contacts}</div>
        </div>
        <div>
          <p class="text-xs font-semibold text-gray-500 mb-2">Tipo da guilda</p>
          <div class="flex flex-wrap gap-2">${guildType}</div>
        </div>
        <div>
          <p class="text-xs font-semibold text-gray-500 mb-2">Foco</p>
...
async function resolveKeyAndLoad(forceMessage = true) {
  ...
  try {
    ...
    if (!keySnap.exists()) {
      ...
      setStatus('Essa chave ainda não existia. Ela foi criada e vinculada à sua conta atual.', 'success');
    }
    ...
    if (!linkedUid) {
      ...
      setStatus('Essa chave foi encontrada, mas ainda está sem uma conta válida vinculada.', 'error');
      renderRecruitment();
      return;
    }
    ...
    if (forceMessage) {
      ...
      else setStatus('Nenhum recrutamento foi encontrado para essa chave no momento.', 'warn');
    }
