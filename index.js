...
let ownerSignupRecoveryUser = null;

function getOwnerSignupRecoveryUser() {
  try {
    const current = auth.currentUser;
    if (!current || !ownerSignupRecoveryUser) return null;
    return current.uid === ownerSignupRecoveryUser.uid ? current : null;
  } catch (_) {
    return null;
  }
}

function startOwnerSignupRecovery(user) {
  if (!user || !user.uid) return false;
  ownerSignupRecoveryUser = user;
  ...
  if (loginHint) loginHint.textContent = 'Complete seu cadastro de dono';

  if (ownerSwitch) {
    ownerSwitch.checked = true;
    ownerSwitch.disabled = true;
  }
  guildWrap?.classList.remove('hidden');
  if (guildInput) guildInput.required = true;
  if (modeDesc) {
    modeDesc.textContent = 'Sua conta ja existe. Complete os dados da guilda para acessar o painel.';
  }

  if (emailInput) {
    emailInput.value = cleanEmail(user.email || '');
  ...
  if (passwordInput) {
    ...
    passwordInput.placeholder = 'Conta ja criada';
    passwordInput.closest('div')?.parentElement?.classList.add('hidden');
  }

  const label = btnSignup?.querySelector('span');
  if (label) label.textContent = 'Completar cadastro';

  showToast('info', 'Encontramos sua conta. Complete o cadastro da guilda para continuar.');
  return true;
}
...
function setupAuthForms() {
  ...
  loginForm?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      console.error(err);
      if (String(err?.message || '') === 'incomplete-account' && auth.currentUser) {
        startOwnerSignupRecovery(auth.currentUser);
  ...
  signupForm?.addEventListener('submit', async (e) => {
    ...
    const recoveryUser = getOwnerSignupRecoveryUser();
    const isRecovery = !!recoveryUser;
    const isOwner = isRecovery ? true : !!qs('signup-is-owner')?.checked;
    ...
    const email = isRecovery ? cleanEmail(recoveryUser.email || '') : String(qs('signup-email')?.value || '').trim();
    const pass = isRecovery ? '' : String(qs('signup-password')?.value || '');
    ...
    setButtonLoading(btn, true, isRecovery ? 'Completando...' : 'Criar conta');
    ...
    try {
      let accountUser = recoveryUser;
      if (!isRecovery) {
        await ensureSignupAvailable(gameId, email);
        const cred = await createUserWithEmailAndPassword(auth, email, pass);
        createdUser = cred.user;
        accountUser = cred.user;
      }

      const result = isOwner
        ? await createOwnerAccount(accountUser, { gameId, guildName, nick })
        : await createPlayerAccount(accountUser, { gameId, nick });
      createdResult = result;
      createdRef = result?.partnerRef || getStoredPartnerRef();

      await verifySignupCreation(accountUser, { gameId, isOwner });

      if (isOwner && createdRef) {
        if (isRecovery) {
          try { await bindPartnerReferralAfterSignup(accountUser, gameId, accountUser.uid, createdRef); }
          catch (refError) { console.warn('[owner-recovery-referral]', refError); }
        } else {
          await bindPartnerReferralAfterSignup(accountUser, gameId, accountUser.uid, createdRef);
        }
      }
      ...
      ownerSignupRecoveryUser = null;
      showToast('success', isRecovery ? 'Cadastro completado com sucesso!' : 'Conta criada com sucesso!');
      try { sessionStorage.setItem('hub_do_preload', '1'); } catch (_) {}
      ...
      if (isRecovery) msg = 'Nao foi possivel completar o cadastro. Confira os dados e tente novamente.';
      ...
      showToast('error', msg);
      setButtonLoading(btn, false, isRecovery ? 'Completar cadastro' : 'Criar conta');
      if (isRecovery) startOwnerSignupRecovery(recoveryUser);
    }
  });
}
